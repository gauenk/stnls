...
def pytest_generate_tests(metafunc):
    ...
    th.manual_seed(seed)
    np.random.seed(seed)
    test_lists = {"ps":[7],"stride0":[4],"stride1":[1,2],
                  "dilation":[1],"wt":[1,2],"ws":[5], "wr":[5],
                  "k":[10,30],"kr":[-1],"exact":[True],"nheads":[1,2],
                  "seed":[0]}
...
def test_fwd(wr,kr,ws,wt,k,ps,stride0,stride1,dilation,nheads,exact,seed):
    ...
    search_te = stnls.search.ApproxTimeSearch(ws, wt, ps, k, wr, kr, nheads,
                                 dilation=dil,stride0=stride0, stride1=stride1,
                                 reflect_bounds=reflect_bounds,full_ws=False,
                                 anchor_self=anchor_self,remove_self=False,
                                 use_adj=use_adj,rbwd=rbwd,nbwd=nbwd,exact=exact)
