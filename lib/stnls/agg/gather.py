...
class non_local_gather(th.autograd.Function):
    """

    Gather the non-local patches according to inds
...
class NonLocalGather(th.nn.Module):
    ...
    def forward(self, vid, weights, inds):
        ...
        stack = non_local_gather.apply(vid, weights, inds, *inputs)
        return stack
...
def init(cfg):
    ...
    search = NonLocalGather(cfg.ps,cfg.stride0,cfg.pt,cfg.reflect_bounds,
