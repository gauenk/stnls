...
Search only the second frame of a two frame input
...
class NLSPairFunction(th.autograd.Function):
    ...
    def forward(ctx, vid0, vid1, fflow, bflow,
        ...
        # -- run batches --
        assert T == 2
        Q = ((H-1)//stride0+1)*((W-1)//stride0+1)
        qshift = Q # skip first frame

        # -- run [optionally batched] forward function --
        dists,inds = nls_fwd_main(vid0, vid1, fflow, bflow,
                                 ws, wt, ps, k, dist_type,
                                 stride0, stride1, dilation, pt,
                                 anchor_self, remove_self, reflect_bounds,
                                 full_ws, full_ws_time, use_adj,
...
class NLSPair(th.nn.Module):
    ...
    def forward(self, vid0, vid1, fflow, bflow, batchsize=-1):
        return NLSPairFunction.apply(vid0,vid1,fflow,bflow,
...
def init(cfg):
    ...
    search = NLSPair(cfg.ws, cfg.wt, cfg.ps, cfg.k, nheads=cfg.nheads,
