...
__global__ void paired_refine_int_forward_kernel(
  ...
  for (int q_index = 0; q_index < q_per_thread; q_index++){
    ...
    // -- assign to reference --
    frame_anchor[0] = ref_patch[0] + flow[ibatch][ihead_fl][si][nh][nw][0];
    frame_anchor[1] = ref_patch[1] + flow[ibatch][ihead_fl][si][nh][nw][1];
    ...
    for (int _xi = 0; _xi < ws_per_thread; _xi++){
      ...
      for (int _yi = 0; _yi < ws_per_thread; _yi++){
        ...
        dists[ibatch][ihead_sr][qi][si][ws_i][ws_j] = dist;
        inds[ibatch][ihead_sr][qi][si][ws_i][ws_j][0] = prop_patch[0]-ref_patch[0];
        inds[ibatch][ihead_sr][qi][si][ws_i][ws_j][1] = prop_patch[1]-ref_patch[1];
  ...
  for (int q_index = 0; q_index < q_per_thread; q_index++){
    ...
    for (int _xi = 0; _xi < ws_per_thread; _xi++){
      ...
      for (int _yi = 0; _yi < ws_per_thread; _yi++){
        ...
        inds[ibatch][ihead_sr][qi][si][ws_i][ws_j][0] = prop_patch[0]-ref_patch[0];
        inds[ibatch][ihead_sr][qi][si][ws_i][ws_j][1] = prop_patch[1]-ref_patch[1];
        kselect[ibatch][ihead][qi][si][ws_i][ws_j] = si;
...
template <typename scalar_t, int DIST_TYPE>
__global__ void paired_refine_int_backward_kernel(
...
void paired_refine_int_backward_cuda(
  ...
  // -- launch kernel --
  if (dist_type == 0){ // prod
    AT_DISPATCH_FLOATING_TYPES(frame0.type(),"paired_refine_backward_kernel", ([&] {
    paired_refine_int_backward_kernel<scalar_t,0><<<blocksPerGrid, threadsPerBlock>>>(
    ...
    AT_DISPATCH_FLOATING_TYPES(frame0.type(),"paired_refine_backward_kernel", ([&] {
    paired_refine_int_backward_kernel<scalar_t,1><<<blocksPerGrid, threadsPerBlock>>>(
          grad_frame0.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
          grad_frame1.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
          frame0.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
          frame1.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
...
template <typename scalar_t, int DIST_TYPE>
__global__ void paired_refine_bilin2d_backward_kernel(
    torch::PackedTensorAccessor32<scalar_t,5,torch::RestrictPtrTraits> grad_frame0,
    torch::PackedTensorAccessor32<scalar_t,5,torch::RestrictPtrTraits> grad_frame1,
  ...
  #pragma unroll
    ...
    scalar_t hi = ref_patch[0] + flow[ibatch][ihead_fl][qi][kj][0];
    scalar_t wi = ref_patch[1] + flow[ibatch][ihead_fl][qi][kj][1];
...
void paired_refine_bilin2d_backward_cuda(
  ...
  if (dist_type == 0){ // prod
    ...
    paired_refine_bilin2d_backward_kernel<scalar_t,0><<<blocksPerGrid, threadsPerBlock>>>(
          grad_frame0.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
          grad_frame1.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
    ...
    AT_DISPATCH_FLOATING_TYPES(frame0.type(),
                               "paired_refine_bilin2d_backward_kernel", ([&] {
    paired_refine_bilin2d_backward_kernel<scalar_t,1><<<blocksPerGrid, threadsPerBlock>>>(
          grad_frame0.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
          grad_frame1.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
          grad_flow.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
          frame0.packed_accessor32<scalar_t,5,torch::RestrictPtrTraits>(),
