...
template <typename scalar_t>
__global__ void non_local_flow_kernel(
    torch::PackedTensorAccessor64<scalar_t,6,torch::RestrictPtrTraits> inds,
    const torch::PackedTensorAccessor64<scalar_t,5,torch::RestrictPtrTraits> fflow,
    const torch::PackedTensorAccessor64<scalar_t,5,torch::RestrictPtrTraits> bflow,
    int ws, int wt, int nH, int nW, int nHW,
  ...
  for (int _qi = 0; _qi < q_per_thread; _qi++){
    ...
    for (int _xi = 0; _xi < ws_per_thread; _xi++){
      ...
      for (int _yi = 0; _yi < ws_per_thread; _yi++){
        ...
        // -- fill --
        prop[0] = ref[0];
        prop[1] = hj + stride1 * (ws_i - wsOff_h);
        prop[2] = wj + stride1 * (ws_j - wsOff_w);
        valid = true;
        // check_bounds(valid,prop,T,H,W);    

        // -- set indices --
        inds[ibatch][qi][0][ws_i][ws_j][0] = valid ? prop[0]-ref[0] : invalid;
        // inds[ibatch][qi][0][ws_i][ws_j][1] = valid ? prop[1]-ref[1] : invalid;
        inds[ibatch][qi][0][ws_i][ws_j][1] = valid ? 10 : invalid;
        inds[ibatch][qi][0][ws_i][ws_j][2] = valid ? prop[2]-ref[2] : invalid;
      }
    }
...
void non_local_flow_cuda(
  ...
  // -- launch kernel --
  AT_DISPATCH_FLOATING_TYPES(fflow.type(), "non_local_flow_kernel", ([&] {
     non_local_flow_kernel<scalar_t><<<nblocks, nthreads>>>(
       inds.packed_accessor64<scalar_t,6,torch::RestrictPtrTraits>(),
       fflow.packed_accessor64<scalar_t,5,torch::RestrictPtrTraits>(),
       bflow.packed_accessor64<scalar_t,5,torch::RestrictPtrTraits>(),
       ws, wt, nH, nW, nHW, stride0, stride1, full_ws,
