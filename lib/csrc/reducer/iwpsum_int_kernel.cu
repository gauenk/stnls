...
__global__ void iwpsum_int_forward_kernel(
    ...
    bool reflect_bounds, int psOffset, int q_per_thread, int f_per_thread){
...
void iwpsum_int_forward_cuda(
  ...
  // -- launch kernel --
  AT_DISPATCH_FLOATING_TYPES(in_vid.type(), "iwpsum_int_forward_kernel", ([&] {
    iwpsum_int_forward_kernel<scalar_t><<<nblocks, nthreads>>>(
        ...
        ps, pt, dilation, reflect_bounds, psOffset,
