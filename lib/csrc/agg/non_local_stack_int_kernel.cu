...
__global__ void non_local_stack_forward_kernel(
    ...
    if ((qi < Q) && (ki < K)){
      ...
      // -- reference index --
      get_pixel_loc(ref_patch,qindex,stride0,nW0,nHW0,height,width);
...
void non_local_stack_forward_cuda(
  ...
  // -- launch kernel --
  AT_DISPATCH_FLOATING_TYPES(vid.type(), "non_local_stack_forward_kernel", ([&] {
        non_local_stack_forward_kernel<scalar_t><<<blocksPerGrid, threadsPerBlock>>>(
...
template <typename scalar_t>
__global__ void non_local_stack_backward_kernel(
...
void non_local_stack_backward_cuda(
  ...
  // -- launch kernel --
  AT_DISPATCH_FLOATING_TYPES(vid.type(), "non_local_stack_backward_kernel", ([&] {
        non_local_stack_backward_kernel<scalar_t><<<blocksPerGrid, threadsPerBlock>>>(
