...
def delta_patch(vid0,vid1,loc0,loc1,ps):
    ...
    for pi in range(ps):
        for pj in range(ps):
            ...
            delta += th.mean((pix0-pix1)**2)
    return delta/ps**2


def search_deltas(vid0,vid1,fflow,bflow,grid,stride1,ws,ps):
    ...
    topk = th.topk(dmap0,K)
    inds = topk.indices
    krank = th.exp(-(1./2)*th.arange(K)/K)
    # print(krank)
    dmap0[topk.indices] = krank
    dmap = dmap.reshape(-1)
    dmap[topk.indices] = 0
...
def main():
    ...
    dmap = search_deltas(clean,clean,fflow,bflow,grid,stride1,ws,ps)
