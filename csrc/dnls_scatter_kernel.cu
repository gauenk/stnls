...
void dnls_cuda_scatter_forward(
  ...
  AT_DISPATCH_FLOATING_TYPES(vid.type(), "dnls_scatter_forward_kernel", ([&] {
    dnls_scatter_forward_kernel<scalar_t><<<blocks, threads>>>(
...
__global__ void dnls_scatter_backward_kernel(
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> grad_patches,
...
void dnls_cuda_scatter_backward(
    torch::Tensor grad_patches,torch::Tensor vid,torch::Tensor nlInds) {
  ...
  AT_DISPATCH_FLOATING_TYPES(vid.type(), "dnls_scatter_backward_kernel", ([&] {
    dnls_scatter_backward_kernel<scalar_t><<<blocks, threads>>>(
        grad_patches.packed_accessor32<scalar_t,4,torch::RestrictPtrTraits>(),
